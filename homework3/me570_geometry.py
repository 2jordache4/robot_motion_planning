...
class Sphere:
    ...
    def distance_grad(self, points):
        ...
        consistently with the definition of Sphere.distance.
        """
        tol = 1e-6
        dist_center = np.linalg.norm(points - self.center, axis=0)
        grad_d_points_sphere = np.zeros(points.shape)

        for i_point in range(0, points.shape[1]):
            if dist_center[i_point] < tol:
                grad_d_points_sphere[:, [i_point]] = np.zeros((2, 1))
            else:
                grad_d_points_sphere[:, [i_point]] = (
                    points - self.center) / dist_center[i_point]
        if self.radius < 0:
