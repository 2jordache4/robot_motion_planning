...
def test_run_plot():
    '''
    Quick function to test potential planner
    '''
    plt.close('all')
    world = pot.SphereWorld()
    planner_parameters = {'epsilon': .0005, 'nb_steps': 600}
    x_goal = world.x_goal[:, 0]
    x_goal = np.reshape(x_goal, (2, 1))
    potential = {'x_goal': x_goal, 'repulsive_weight': 1, 'shape': 'quadratic'}
    links_pot = TwoLinkPotential(world, potential)
    links_pot.run_plot(planner_parameters['epsilon'],
                       planner_parameters['nb_steps'])
