...
from me570_queue import PriorityQueue
...
class Graph:
    ...
    def plot(self,
        ...
        if flag_backpointers_cost and self.has_backpointers:
            ...
            for (cost, coord) in backpointer_cost_list:
                plot_text(coord + offset, 'g=' + str(cost), color=(.8, 1., .8))
    ...
    def expand_element(self, idx_n_best: int, idx_x: int, idx_goal: int,
                       pq_open: PriorityQueue) -> PriorityQueue:
        """
        ...
        # f(n) = g(n) + h(n) --> PriorityQueue Queue
    ...
    def path(self, idx_start, idx_goal):
        ...
        # if start and goal indeces are the same, there is nothing left to do
        # otherwise:
        if idx_start != idx_goal:
            ...
            # Make sure to grab the final element which will be the start vector
            # since the while loop will terminate one step early
            x_path = np.hstack((self.graph_vector[curr_bp]['x'], x_path))
    ...
    def search(self,
               idx_start,
               idx_goal,
               flag_debug_visualization=False,
               save_expansions=None):
        """
        Implements the  A^* algorithm, as described by the pseudo-code in Algorithm 1.
        """
        ...
        i_iter = 0
        i_max = 100000
        if save_expansions is not None:
            save_cnt = 0
        while True:
            idx_best, _ = priority_queue.min_extract()
            if idx_best is None:
                break
            self.idx_closed.append(idx_best)
            if flag_debug_visualization:
                self.plot(idx_closed=self.idx_closed, idx_best=idx_best)
                plot_wait()

            if idx_best == idx_goal:
                break

            unexplored_neighbors = self.get_expand_list(
                idx_best, self.idx_closed)
            ...
            for _, neighbor_node in enumerate(unexplored_neighbors):
                if save_expansions is not None:
                    save_data = {
                        ...
                        tuple(
                            copy.deepcopy(x)
                            for x in (self, idx_best, neighbor_node, idx_goal,
                                      priority_queue)),
                    }
                priority_queue = self.expand_element(idx_best, neighbor_node,
                                                     idx_goal, priority_queue)
                if save_expansions is not None:
                    save_data['outputs'] = (copy.deepcopy(self),
                                            priority_queue)
                    filename = save_expansions % (save_cnt)
                    save_cnt += 1
                    with open(filename, 'wb') as fout:
                        pickle.dump(save_data, fout)
                        print(f'Saving expand_element data to {filename}')
...
function.
    ...
    def __init__(self, nb_cells):
        ...
        def f_handle_total(point):
            return not isnan(world_potential.eval(point))
...
def test_grid2graph():
    ...
    def func(x_eval):
        cond = [
            x_eval[[1]] > pi / 2,
            np.linalg.norm(x_eval - np.ones((2, 1))) < 0.75,
            np.linalg.norm(x_eval - np.array([[4], [2.5]])) >= 0.5
        ]
        return (cond[0] or cond[1]) and cond[2]
