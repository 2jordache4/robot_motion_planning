...
def polygon_is_visible_test_plot(polygon, test_points):
    """
    Helper function for polygon_is_visible_test to run tests and plot the
...
def polygon_is_visible_test():
    ...
    for polygon in robot.polygons:
        polygon_is_visible_test_plot(polygon, test_points)
        polygon.flip()
        polygon_is_visible_test_plot(polygon, test_points)
...
def priority_test():
    ...
    - Add three elements (as shown in Table~tab:priority-test-inputs and in that order) to that
    queue.
    - Extract a minimum element.
    - Add another element (as shown in Table~tab:priority-test-inputs).
    - Check if an element is present.
    - Remove all elements by repeated extractions.  enumerate After each step, display the content
    of pQueue.
    """
